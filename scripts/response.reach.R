...
create.subdirs(proj.dir, c(basesubdir, "by.reach", "pred.fish"))
create.subdirs(proj.dir,c(basesubdir, "by.reach", "pred.fish_perLength_m"))
create.subdirs(proj.dir,c(basesubdir, "by.reach","pred.fish_perArea_m2"))
...
site.fish = site.fish.metrics %>% 
  filter(model == model & species == species & lifestage == lifestage & var == "pred.fish")
...
OUTdir=paste(proj.dir, paste(c(basesubdir, "by.reach" ,"pred.fish"), collapse="\\"), sep="\\")
...
OUTdir=paste(proj.dir, paste(c(basesubdir, "by.reach","pred.fish_perLength_m"), collapse="\\"), sep="\\")
#delete any existing files in Output from previous runs
unlink(paste(OUTdir,"\\*", sep=""), recursive=T)
#set data
...
#specify output directory
OUTdir=paste(proj.dir, paste(c(basesubdir,"by.reach", "pred.fish_perArea_m2"), collapse="\\"), sep="\\")
#delete any existing files in Output from previous runs
unlink(paste(OUTdir,"\\*", sep=""), recursive=T)
#set data
...
pred.fish=site.fish.metrics[which(site.fish.metrics$category=="reach" & site.fish.metrics$var=="pred.fish" & site.fish.metrics$layer==model & site.fish.metrics$species==species),]
mainthalweg=site.fish.metrics%>%filter(category=="main" & var=="length")
hydroarea=site.fish.metrics[which(site.fish.metrics$category=="reach" & site.fish.metrics$var=="area" & site.fish.metrics$layer==model & site.fish.metrics$species==species),]
...
#save
ggsave(paste(OUTdir,"\\by_width.bankfull_m", sep=""), plot=h, width = 15, height = 5 )
ggsave(paste(OUTdir,"\\by_depth.bankfull_m", sep=""), plot=i, width = 15, height = 5 )
ggsave(paste(OUTdir,"\\by_length.hydro_m", sep=""), plot=j, width = 15, height = 5 )
ggsave(paste(OUTdir,"\\by_volume.as.hydroarea.bfdepth_m3", sep=""), plot=k, width = 15, height = 5 )
ggsave(paste(OUTdir,"\\by_braid.index.champ", sep=""), plot=l, width = 15, height = 5 )
ggsave(paste(OUTdir,"\\by_area.hydro_m2", sep=""), plot=m, width = 15, height = 5 )
